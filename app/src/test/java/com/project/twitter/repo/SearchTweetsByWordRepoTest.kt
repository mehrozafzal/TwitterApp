...
class SearchTweetsByWordRepoTest {
    ...
    @Test
    fun testSearchTweetsByWordResponse() {
        val responseBody =
            """
           { "statuses": [
        {
    ...
    "search_metadata": {
        ...
        apiService.getTweetsByTag("#competition")
        ...
        assertThat(
            request.requestLine,
            equalTo("GET /1.1/search/tweets.json?q=%23competition HTTP/1.1")
        );
        Assert.assertNull(error)
        Assert.assertTrue(twitterSearchResponse?.statuses?.get(0)?.text?.contains("#competition")!!)
